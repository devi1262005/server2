...
for i in range(100):
    ...
    # Loans, mortgages, bills, EMIs, transactions
    transactions = []
    loans, mortgages, bills, emis = [], [], [], []
    total_spent, missed_payments = 0, 0

    for _ in range(20):
        transactions.append({"amount": random.randint(500, 5000), "transaction_type": random.choice(["sent", "received"])})
        ...
        loan_status = random.choice([True, False])
        loans.append({"amount": random.randint(50000, 500000), "repaid": loan_status})
        mortgages.append({"amount": random.randint(100000, 1000000), "repaid": loan_status})
        if not loan_status:
            missed_payments += 1
    ...
    debt_ratio = (sum([m["amount"] for m in mortgages if not m["repaid"]]) + sum([b["amount"] for b in bills if not b["paid"]]) + sum([e["amount"] for e in emis if not e["paid"]])) / (card["balance"] + 1)
